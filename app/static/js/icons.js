/* FuelLog — petite bibliothèque d'icônes SVG (style « trait », dans le thème).
   Les icônes héritent de la couleur du texte (stroke=currentColor) : elles
   prennent donc automatiquement l'accent de l'élément (jaune/vert/orange/violet,
   ou la couleur active des onglets). Remplace les emojis partout dans l'UI.

   - Statique (templates) : <span data-ic="wrench"></span> → hydraté au chargement.
   - Dynamique (JS)        : svgIcon('wrench')  → renvoie le markup SVG. */
(function () {
  var P = {
    ...
    car:      '<path d="M5 17H3v-5l2-5h11l3 5h2v5h-2"/><path d="M9 17h6"/><circle cx="7" cy="17" r="2"/><circle cx="17" cy="17" r="2"/>',
  ...
  function hydrate(root) {
    (root || document).querySelectorAll('[data-ic]').forEach(function (el) {
      if (el.__ic) return;                 // déjà fait
      el.__ic = 1;
      var opts = {};
      if (el.dataset.icFill === '1') opts.fill = 'currentColor';
      el.innerHTML = svgIcon(el.dataset.ic, opts);
    });
  }
