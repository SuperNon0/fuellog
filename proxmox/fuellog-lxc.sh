...
#
# ============================================================================
#  FuelLog (autonome) — déploiement d'un conteneur LXC sur Proxmox VE
#
#  À COLLER DANS LE SHELL DE L'HÔTE PROXMOX (pas dans un conteneur).
#  Crée un LXC Debian 12, le démarre, y installe FuelLog et le lance
...
#      ADMIN_PASSWORD='monMotDePasse' bash fuellog-lxc.sh
# ============================================================================
set -euo pipefail
...
pct create "$CTID" "local:vztmpl/$TEMPLATE" \
  ...
  --onboot 1 >/dev/null
ok "Conteneur créé (redémarre avec l'hôte)"
...
pct exec "$CTID" -- bash -c "apt-get update -qq && apt-get install -y -qq curl >/dev/null && \
  BRANCH='$BRANCH' PORT='$PANEL_PORT' REPO_URL='$REPO_URL' ADMIN_PASSWORD='$ADMIN_PASSWORD' \
  bash <(curl -fsSL https://raw.githubusercontent.com/SuperNon0/fuellog/$BRANCH/install.sh)"

echo ""
ok "Terminé — voir le résumé (adresse + mot de passe) ci-dessus."
echo "   Console : pct enter $CTID    ·    Logs : pct exec $CTID -- journalctl -u fuellog -f"
echo "   Pense à placer l'accès derrière Cloudflare Access / un VPN pour l'exposer."
